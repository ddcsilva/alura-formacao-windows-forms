...
public class ChecaForcaSenha
{
    ...
    public ForcaDaSenhaEnum GetForcaDaSenha(string senha)
    {
        int placar = geraPontosSenha(senha);

        if (placar < 50)
            return ForcaDaSenhaEnum.Inaceitavel;
        else if (placar < 60)
            return ForcaDaSenhaEnum.Fraca;
        else if (placar < 80)
            return ForcaDaSenhaEnum.Aceitavel;
        else if (placar < 100)
            return ForcaDaSenhaEnum.Forte;
        else
            return ForcaDaSenhaEnum.Segura;
    }
}